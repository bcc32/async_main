...
*)
let in_async ?extract_exn on_result staged_main =
  stage (fun () ->
...
module Staged = struct
  let async ?extract_exn staged_main =
    ...
    in_async ?extract_exn on_result staged_main
  ;;

  let async_or_error ?extract_exn staged_main =
    let on_result res = maybe_print_error_and_shutdown (Or_error.join res) in
    in_async ?extract_exn on_result staged_main
  ;;
end

let stage_param main () = stage (fun `Scheduler_started -> main ())
let async ?extract_exn param = Staged.async ?extract_exn (stage_param param)

let async_or_error ?extract_exn param =
  Staged.async_or_error ?extract_exn (stage_param param)
;;
